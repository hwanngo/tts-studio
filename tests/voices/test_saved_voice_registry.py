...
def test_saved_voice_service_copies_and_consumes_validated_reference(tmp_path: Path) -> None:
    ...
    reference = references.create_upload(
        model_id="model-one", payload=b"reference", transcript="hello"
    )
    references.mark_validated(
        reference.id,
        ReferenceMetadata(container="wav", sample_rate_hz=16000, channels=1, duration_ms=1),
    )
    service = SavedVoiceService(SavedVoiceRegistry(database), references, layout)

    voice = service.create_from_reference(
...
def test_saved_voice_copy_rejects_replaced_reference_content(
...
def test_saved_voice_destination_swap_fails_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    layout = StorageLayout.from_root(tmp_path / "data")
    layout.ensure()
...
def test_saved_voice_creation_retains_row_and_file_when_row_rollback_fails(
    ...
    monkeypatch.setattr(registry, "delete", fail_delete)
    monkeypatch.setattr(
        references, "delete", lambda reference_id: (_ for _ in ()).throw(OSError("cleanup"))
    )
    service = SavedVoiceService(registry, references, layout)
