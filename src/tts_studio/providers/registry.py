...
class ProviderRegistry:
    ...
    def create(self, *, kind: str, label: str, base_url: str, model: str, api_key_env: str,
               provider_id: str | None = None) -> ProviderProfile:
        validate_provider_input(kind=kind, label=label, base_url=base_url, model=model, api_key_env=api_key_env)
        ...
        with self._database.transaction() as connection:
            connection.execute(
                """INSERT INTO provider_profiles
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (identifier, kind, label.strip(), base_url.rstrip("/"), model.strip(), api_key_env, now, now),
            )
            row = connection.execute("SELECT * FROM provider_profiles WHERE id = ?", (identifier,)).fetchone()
    ...
    def list(self) -> tuple[ProviderProfile, ...]:
        with self._database.read() as connection:
            rows = connection.execute("SELECT * FROM provider_profiles ORDER BY created_at, id").fetchall()
    ...
    def update(self, provider_id: str, **values: str) -> ProviderProfile:
        ...
        merged = {"kind": current.kind, "label": current.label, "base_url": current.base_url,
                  "model": current.model, "api_key_env": current.api_key_env, **values}
        ...
        with self._database.transaction() as connection:
            connection.execute(
                """UPDATE provider_profiles
                   ...
                   WHERE id = ?""",
                (merged["kind"], merged["label"].strip(), merged["base_url"].rstrip("/"),
                 merged["model"].strip(), merged["api_key_env"], now, provider_id),
            )
            row = connection.execute("SELECT * FROM provider_profiles WHERE id = ?", (provider_id,)).fetchone()
        return _profile(_required(row))
    ...
    def delete(self, provider_id: str) -> None:
        with self._database.transaction() as connection:
            _required(connection.execute("SELECT id FROM provider_profiles WHERE id = ?", (provider_id,)).fetchone())
