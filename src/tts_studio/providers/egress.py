"""Core-side validation for remote provider destinations."""
...
class ProviderEgressError(ValueError):
    """Provider target is outside the permitted Core egress policy."""
...
def resolve_provider_addresses(url: str, *, resolve_dns: bool = True) -> tuple[str, ...]:
    """Resolve and validate a provider target without depending on Worker packages."""
    parsed = urlsplit(url)
    if (
