...
_ALIGNMENT_STATES = {AlignmentState.QUEUED, AlignmentState.RUNNING, AlignmentState.COMPLETED, AlignmentState.FAILED}
_TRANSITIONS = {
    GenerationState.QUEUED: {GenerationState.LOADING, GenerationState.CANCELLED, GenerationState.FAILED},
    GenerationState.LOADING: {GenerationState.GENERATING, GenerationState.CANCELLED, GenerationState.FAILED},
    GenerationState.GENERATING: {GenerationState.FINALIZING, GenerationState.CANCELLED, GenerationState.FAILED},
...
class GenerationRegistry:
    ...
    def create_job(
        ...
        *,
        ...
        with self._database.transaction() as connection:
            ...
            row = _required_row(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (durable_id,)))
        return _job(row)
    ...
    def clear_reference_id(self, job_id: str) -> GenerationJob:
        with self._database.transaction() as connection:
            _job_or_missing(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)).fetchone(), job_id)
            connection.execute(
                "UPDATE generation_jobs SET reference_id = NULL, updated_at = ? WHERE id = ?",
                (_utc_now(), job_id),
            )
            row = _required_row(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)))
        return _job(row)

    def list_jobs(self) -> tuple[GenerationJob, ...]:
        with self._database.read() as connection:
            rows = connection.execute("SELECT * FROM generation_jobs ORDER BY created_at, id").fetchall()
    ...
    def request_cancellation(self, job_id: str) -> GenerationJob:
        with self._database.transaction() as connection:
            ...
            return _job(_required_row(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,))))
        ...
        *,
        ...
        with self._database.transaction() as connection:
            current = _job_or_missing(
                connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)).fetchone(), job_id
            )
            ...
            if current.cancellation_requested and state not in {GenerationState.CANCELLED, GenerationState.FAILED}:
                raise InvalidGenerationTransitionError(
            ...
            connection.execute(
                ...
                (state.value, next_bytes, next_frames, sample_rate, channel_count, error_json, _utc_now(), job_id),
            )
            return _job(_required_row(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,))))
    ...
    def create_artifact(
        ...
        *,
        ...
        now = _utc_now()
        with self._database.transaction() as connection:
            job = _job_or_missing(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)).fetchone(), job_id)
            ...
            connection.execute(
                """INSERT INTO audio_artifacts
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (durable_id, job_id, safe_path, byte_size, sha256, sample_rate, channel_count, frame_count, duration_ms, now, now),
            )
            connection.execute("UPDATE generation_jobs SET artifact_id = ?, updated_at = ? WHERE id = ?", (durable_id, now, job_id))
            row = _required_row(connection.execute("SELECT * FROM audio_artifacts WHERE id = ?", (durable_id,)))
        return _artifact(row)

    def get_alignment(self, job_id: str) -> AlignmentJob:
        with self._database.transaction() as connection:
            job = _job_or_missing(
                connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)).fetchone(),
                job_id,
            )
            ...
            if row is None:
                raise GenerationRecordNotFoundError(f"Alignment for Generation Job {job_id!r} was not found")
            if not _alignment_matches_current_artifact(connection, job, row):
                ...
                raise GenerationRecordNotFoundError(f"Alignment for Generation Job {job_id!r} was stale")
    ...
    def create_alignment(self, job_id: str) -> AlignmentJob:
        ...
        with self._database.transaction() as connection:
            ...
            artifact = connection.execute(
                "SELECT id FROM audio_artifacts WHERE id = ? AND job_id = ?", (job.artifact_id, job_id)
            ).fetchone()
            if artifact is None:
                raise InvalidRegistryDataError("alignment requires a retained Audio Artifact")
            existing = connection.execute("SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,)).fetchone()
            ...
            return _alignment(_required_row(connection.execute("SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,))))

    def retry_alignment(self, job_id: str) -> AlignmentJob:
        with self._database.transaction() as connection:
            current = _alignment_or_missing(connection.execute("SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,)).fetchone(), job_id)
            if current.state is not AlignmentState.FAILED or current.error is None or current.error.get("retryable") is not True:
                raise InvalidRegistryDataError("alignment failure is not retryable")
            job = _job_or_missing(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)).fetchone(), job_id)
            if job.state is not GenerationState.COMPLETED or not job.retain_artifact or job.artifact_id != current.artifact_id:
                raise InvalidRegistryDataError("alignment requires a completed Generation Job with its retained Audio Artifact")
            artifact = connection.execute("SELECT id FROM audio_artifacts WHERE id = ? AND job_id = ?", (current.artifact_id, job_id)).fetchone()
            if artifact is None:
                raise InvalidRegistryDataError("alignment requires a retained Audio Artifact")
            connection.execute(
    ...
    def start_alignment(self, job_id: str) -> AlignmentJob:
        with self._database.transaction() as connection:
            job = _job_or_missing(connection.execute("SELECT * FROM generation_jobs WHERE id = ?", (job_id,)).fetchone(), job_id)
            row = connection.execute("SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,)).fetchone()
            if row is None:
                raise GenerationRecordNotFoundError(f"Alignment for Generation Job {job_id!r} was not found")
            if not _alignment_matches_current_artifact(connection, job, row):
                connection.execute("DELETE FROM generation_alignments WHERE job_id = ?", (job_id,))
            ...
            if current.state is AlignmentState.QUEUED:
                connection.execute("UPDATE generation_alignments SET state = 'running', updated_at = ? WHERE job_id = ?", (_utc_now(), job_id))
    ...
    def complete_alignment(self, job_id: str, result: AlignmentResult) -> AlignmentJob:
        ...
        with self._database.transaction() as connection:
            ...
            if current.state not in {AlignmentState.QUEUED, AlignmentState.RUNNING}:
                raise InvalidRegistryDataError("alignment is terminal")
            artifact = connection.execute("SELECT sample_rate, frame_count, id FROM audio_artifacts WHERE id = ? AND job_id = ?", (job.artifact_id, job_id)).fetchone() if job.artifact_id else None
            if artifact is None:
                raise InvalidRegistryDataError("alignment requires a retained Audio Artifact")
            _validate_alignment_result(result, job, artifact)
            connection.execute("UPDATE generation_alignments SET state = 'completed', result_json = ?, error_json = NULL, updated_at = ? WHERE job_id = ?", (result_json, _utc_now(), job_id))
            return _alignment(_required_row(connection.execute("SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,))))
    ...
    def fail_alignment(self, job_id: str, error: JsonObject) -> AlignmentJob:
        ...
        with self._database.transaction() as connection:
            ...
            if current.state not in {AlignmentState.QUEUED, AlignmentState.RUNNING}:
                raise InvalidRegistryDataError("alignment is terminal")
            connection.execute("UPDATE generation_alignments SET state = 'failed', result_json = NULL, error_json = ?, updated_at = ? WHERE job_id = ?", (error_json, _utc_now(), job_id))
            return _alignment(_required_row(connection.execute("SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,))))
    ...
    def recover_alignment(self, job_id: str) -> AlignmentJob:
        ...
        with self._database.transaction() as connection:
            ...
            return _alignment(_required_row(connection.execute(
                "SELECT * FROM generation_alignments WHERE job_id = ?", (job_id,)
            )))

    def list_history(self) -> tuple[AudioArtifact, ...]:
        with self._database.read() as connection:
            rows = connection.execute("SELECT * FROM audio_artifacts ORDER BY retained_at DESC, id DESC").fetchall()
        return tuple(_artifact(row) for row in rows)

    def delete_artifact(self, artifact_id: str) -> bool:
        with self._database.transaction() as connection:
            row = connection.execute("SELECT job_id FROM audio_artifacts WHERE id = ?", (artifact_id,)).fetchone()
            if row is None:
                return False
            connection.execute("DELETE FROM audio_artifacts WHERE id = ?", (artifact_id,))
            connection.execute("UPDATE generation_jobs SET artifact_id = NULL, updated_at = ? WHERE id = ?", (_utc_now(), row[0]))
            return True
    ...
    def mark_recovery_failure(self, job_id: str, error: JsonObject) -> GenerationJob:
        ...
        if current.state not in _ACTIVE_STATES and current.state is not GenerationState.QUEUED:
            raise InvalidGenerationTransitionError(f"only an active Generation Job can fail recovery, not {current.state.value}")
...
def _validate_alignment_shape(result: AlignmentResult) -> None:
    if not isinstance(result, AlignmentResult) or type(result.schema_version) is not int or result.schema_version != 1:
    ...
    if not isinstance(result.artifact_id, str) or not result.artifact_id.strip() or "\0" in result.artifact_id:
        raise InvalidRegistryDataError("artifact_id is invalid")
    if not isinstance(result.transcript, str) or len(result.transcript) > _MAX_ALIGNMENT_TRANSCRIPT or "\0" in result.transcript:
    ...
    for unit in result.units:
        if not isinstance(unit, AlignmentUnit) or not isinstance(unit.text, str) or not unit.text or "\0" in unit.text:
            raise InvalidRegistryDataError("alignment unit text is invalid")
        if type(unit.source_start) is not int or type(unit.source_end) is not int or not 0 <= unit.source_start < unit.source_end <= len(transcript_bytes) or unit.source_start < previous_source_end:
        ...
        if type(unit.start_frames) is not int or type(unit.end_frames) is not int or not 0 <= unit.start_frames <= unit.end_frames <= result.total_frames or unit.start_frames < previous_frame_end:
            raise InvalidRegistryDataError("alignment frame range is invalid")
        if not isinstance(unit.confidence, (int, float)) or isinstance(unit.confidence, bool) or not math.isfinite(unit.confidence) or not 0 <= unit.confidence <= 1:
...
def _encode_alignment_result(result: AlignmentResult) -> str:
    _validate_alignment_shape(result)
    payload = {"schema_version": result.schema_version, "job_id": result.job_id, "artifact_id": result.artifact_id, "transcript": result.transcript, "sample_rate_hz": result.sample_rate_hz, "total_frames": result.total_frames, "unit": result.unit, "aligner": result.aligner, "units": [{"text": item.text, "source_start": item.source_start, "source_end": item.source_end, "start_frames": item.start_frames, "end_frames": item.end_frames, "confidence": item.confidence, "estimated": item.estimated} for item in result.units]}
    try:
...
def _decode_alignment_result(value: str | None) -> AlignmentResult | None:
    ...
    try:
        ...
        result = AlignmentResult(units=units, **{key: payload[key] for key in ("schema_version", "job_id", "artifact_id", "transcript", "sample_rate_hz", "total_frames", "unit", "aligner")})
...
def _job(row: Row) -> GenerationJob:
    return GenerationJob(
        id=row["id"], model_id=row["model_id"], engine_id=row["engine_id"], voice_id=row["voice_id"],
        reference_id=row["reference_id"],
        saved_voice_id=row["saved_voice_id"],
        provider_id=row["provider_id"],
        text=row["text"],
        retain_artifact=bool(row["retain_artifact"]),
        options=(SynthesisOptions(speed=row["speed"], pitch=row["pitch"], volume=row["volume"])
                 if any(row[name] is not None for name in ("speed", "pitch", "volume")) else None),
        state=GenerationState(row["state"]),
        bytes_written=row["bytes_written"], frame_count=row["frame_count"], sample_rate=row["sample_rate"],
        channel_count=row["channel_count"], artifact_id=row["artifact_id"], correlation_id=row["correlation_id"],
        cancellation_requested=bool(row["cancellation_requested"]), error=_decode_optional_object(row["error_json"]),
...
def _artifact(row: Row) -> AudioArtifact:
    return AudioArtifact(
        id=row["id"], job_id=row["job_id"], path=row["path"], byte_size=row["byte_size"], sha256=row["sha256"],
        sample_rate=row["sample_rate"], channel_count=row["channel_count"], frame_count=row["frame_count"],
        duration_ms=row["duration_ms"], created_at=row["created_at"], retained_at=row["retained_at"],
    )
...
def _managed_audio_path(value: str) -> str:
    ...
    if not value or "\\" in value or value.startswith("/") or path.parts[:1] != ("audio",) or len(path.parts) < 2 or any(part in {"", ".", ".."} for part in path.parts):
        raise InvalidRegistryDataError("path must be a normalized managed-relative path below audio/")
...
def _encode_optional_object(value: JsonObject | None, field: str, *, max_bytes: int | None = None) -> str | None:
    if value is None:
        return None
